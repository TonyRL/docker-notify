...
if (
    !notifyServices.every((o) =>
        o.actions.every((o2) =>
            o2.type === "webHook"
                ? config.webHooks[o2.instance]
                : o2.type === "mailHook"
                ? config.smtpServer[o2.instance]
                : false
        )
    )
) {
...
const mailHookSend = function (smtpserver, recipient, updatedString, msg) {
    if (!mailtransporterMap.has(smtpserver)) {
        mailtransporterMap.set(
            smtpserver,
            mailService(
                config.smtpServer[smtpserver].host,
    ...
    sendMail(
        msg,
        mailtransporterMap.get(smtpserver),
        config.smtpServer[smtpserver].sendername,
        config.smtpServer[smtpserver].senderadress,
        recipient,
        updatedString
    );
};
    ...
    return new Promise((resolve, reject) => {
        ...
        if (repository.tag) {
            getTagInfo(repository.user, repository.name, token)
                .then((tags) => {
                    const tagInfo = tags.filter(
                        (elem) => elem.name == repository.tag
                    )[0];

                    if (tagInfo == undefined) {
                        logger.error(
                            "Cannot find tag for repository: ",
                            repository.name
                        );
                        return;
                    }
...
const checkForUpdates = async () => {
    dockerHubToken =
        config.dockerHubUsername && config.dockerHubPassword
            ? await dockerAPI.token(
    ...
    Cache.getCache()
        .then((cache) => {
            ...
            for (const job of notifyServices) {
                ...
                repoChecks.push(
                    checkRepository(job, cache[key], dockerHubToken)
                );
            }
