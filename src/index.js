...
const ajv =
    new Ajv({allErrors : true, allowUnionTypes : true, useDefaults : true});
...
if (!notifyServices.every(
        (o) => o.actions.every(
            (o2) => o2.type === 'webHook'    ? config.webHooks[o2.instance]
                    : o2.type === 'mailHook' ? config.smtpServer[o2.instance]
                                             : false))) {
...
const mailHookSend = function(smtpserver, recipient, updatedString, msg) {
  if (!mailtransporterMap.has(smtpserver)) {
    mailtransporterMap.set(smtpserver,
                           mailService(config.smtpServer[smtpserver].host,
                                       config.smtpServer[smtpserver].port,
                                       config.smtpServer[smtpserver].secure,
                                       config.smtpServer[smtpserver].username,
                                       config.smtpServer[smtpserver].password));
  }

  sendMail(msg, mailtransporterMap.get(smtpserver),
           config.smtpServer[smtpserver].sendername,
           config.smtpServer[smtpserver].senderadress, recipient,
           updatedString);
};

// sends an email with a given message to the receiver which is defined in the
// env
const sendMail = function(msg, mailTransporter, smtpSenderName,
                          smtpSenderAddress, mailReceiver, updatedString) {
  mailTransporter.verify()
      .then(() => {
        ...
        mailTransporter.sendMail(mailOptions)
            .then((info) => { logger.log('Notification mail sent: ', info); })
            .catch(
                (err) => { logger.error('Error while sending mail: ', err); });
      })
      .catch((err) => {
...
const getRepositoryInfo = (user, name, token) =>
    dockerAPI.repository(user, name, token);
...
const checkRepository = function(job, repoCache, token) {
  return new Promise((resolve, reject) => {
    ...
    if (repository.tag) {
      getTagInfo(repository.user, repository.name, token)
          .then((tags) => {
            const tagInfo =
                tags.filter((elem) => elem.name == repository.tag)[0];
      ...
      getRepositoryInfo(repository.user, repository.name, token)
          .then(checkUpdateDates)
          .catch((err) => {
...
const checkForUpdates = async () => {
  dockerHubToken = config.dockerHubUsername && config.dockerHubPassword
                       ? await dockerAPI.token(config.dockerHubUsername,
                                               config.dockerHubPassword)
                       : null;
  ...
  Cache.getCache()
        ...
        Promise.all(repoChecks)
            .then((checkResult) => {
              ...
              for (const res of checkResult) {
                ...
                if (res.updated) {
                  let updatedString = res.user == 'library'
                                          ? res.name
                                          : res.user + '/' + res.name;
                  if (res.tag) {
              ...
              Cache.writeCache(JSON.stringify(newCache))
                  .then(() => {
                    if (updatedRepos.length > 0) {
                      updatedRepos.forEach(
                          (o) => o.job.actions.forEach((o2) => {
                            if (o2.type == 'webHook') {
                              ...
                              axios({
                                ...
                                data : webHook.httpBody
                                  ...
                                  .then((body) => {
                                    logger.log('WebHook Action for image [' +
                                                   JSON.stringify(o.job.image) +
                                                   '] successfully. Response: ',
                                               body);
                                  })
                                  .catch((err) => {
                                    logger.error('WebHook Action for image [' +
                                                 JSON.stringify(o.job.image) +
                                                 '] failed');
                                    logger.log(err);
                                  });
                              ...
                              mailHookSend(
                                  o2.instance, o2.recipient, o.updatedString,
                                  'Docker image \'' + o.updatedString +
                                      '\' was updated:\n' +
                                      JSON.stringify(o.job.image, null, 2));
                            } else {
                              logger.error(
                                  'Trying to execute an unknown hook(' +
                                  o2.type +
                                  '), falling back to printing to console');
                              logger.error('Image: ' +
                                           JSON.stringify(o.job.image));
                            }
                          }));
      ...
      .catch((err) => { logger.error('Cannot open cache: ', err); });
};
