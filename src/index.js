...
if (!notifyServices.every(
        (o) => o.actions.every(
            (o2) => o2.type === 'webHook'    ? config.webHooks[o2.instance]
                    ...
                    : o2.type === 'twitter'  ? config.twitter[o2.instance]
                                             : false))) {
...
const getRepositoryInfo = function(
    user, name) { return dockerAPI.repository(user, name); };

const getTagInfo = function(user, name) { return dockerAPI.tags(user, name); };
...
const checkRepository = function(job, repoCache) {
  return new Promise((resolve, reject) => {
    ...
    if (repository.tag) {
      getTagInfo(repository.user, repository.name)
          .then((tags) => {
            const tagInfo =
      ...
      getRepositoryInfo(repository.user, repository.name)
                      ...
                      updatedRepos.forEach(
                          (o) => o.job.actions.forEach((o2) => {
                            if (o2.type == 'webHook') {
                              ...
                              (async () => {
                                try {
                                  message = twitter.message.replace(
                                      "$msg", 'Docker image \'' +
                                                  o.updatedString +
                                                  '\' was updated');
